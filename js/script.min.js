...
function createStates() {
  var projection = albersUsa();
  var path = index$1().projection(projection);
  ...
  json("data/us.json", function (error, us) {
    ...
    .data(feature(us, us.objects.states).features).enter().append("path").attr("d", path).attr("class", "subunit-boundary").attr("fill", MAP_COLOR).attr("stroke", STATE_BORDER_COLOR);
...
function update(chosenLocation) {
  ...
  function drawLocationCircles(locations, settings) {
    ...
    var update = locationGroup.selectAll('circle').data(locations, d => d.name);

    // Remove old circles
    update.exit().transition(t).attr("r", 0).remove();

    // Add new circles in
    update.enter().append('circle').attr("class", "location").attr("r", 0).attr("cx", d => d.x).attr("cy", d => d.y).style("fill", FILL_COLOR).style("stroke", "#252525").transition(t).delay(update.exit().size() ? 1000 : 0).attr("r", d => settings.radiusRange(d.weight));
  }

  function drawLocationText(locations, settings) {
    var t = transition().duration(1000);

    var update = locationGroup.selectAll('text').data(locations, d => d.name);
  ...
  function drawBetweenLines(lineData, settings) {
    ...
    update.exit().transition(t).attr("stroke-width", 0).remove();

    update.enter().append("path").attr("d", d => settings.lineFunction(d.lineData)).attr("stroke-width", 0).attr("fill", "none").transition(t).delay(update.exit().size() ? 1000 : 0).attr("stroke", d => d.target.color).attr("stroke-width", d => settings.lineWidthRange(+d.target.weight) + "px");
  }
