...
function drawLabels({ data, svg, store, maxDomain }) {
	...
	var update = locationLabelGroup.selectAll("text").data(data, d => d.name);
	...
	update.enter().append("text").attr("class", "location-text").attr("x", d => d.x).attr("y", d => d.y).attr("dx", ".5em").attr("dy", "1em").attr("font-size", "0.1em").transition(t).delay(update.exit().size() ? 1000 : 0).attr("font-size", d => d.name === chosenLocation ? "1em" : fontRange(d.weight) >= 10 ? "1em" : `${fontRange(d.weight) / 10}em`).text(d => {
