...
function update(chosenLocation) {
  ...
  function drawData(locations, links) {
    ...
    // .attr("r", d => 3)
    .attr("cx", d => d.x).attr("cy", d => d.y).style("fill", FILL_COLOR)
    // .text(d => d.name)
    ...
    locationText.enter().append("text").attr("class", "location-text").attr("x", d => d.x).attr("y", d => d.y).attr("dx", ".5em").attr("dy", "1em").attr("font-size", d => d.name === chosenLocation ? "1em" : fontRange(d.weight) >= 10 ? "1em" : `${fontRange(d.weight) / 10}em`).text(d => {
