...
var symbol = function() {
  var type = constant$10(symbolCircle),
      size = constant$10(64),
      context = null;
  ...
  function newParse(specifier, newDate) {
    return function(string) {
      ...
      if ("W" in d || "U" in d) {
        ...
        var day$$1 = "Z" in d ? utcDate$1(newYear$1(d.y)).getUTCDay() : newDate(newYear$1(d.y)).getDay();
        d.m = 0;
        d.d = "W" in d ? (d.w + 6) % 7 + d.W * 7 - (day$$1 + 5) % 7 : d.w + d.U * 7 - (day$$1 + 6) % 7;
      }
...
var selection_data$1 = function(value, key) {
  ...
  if (typeof value !== "function") value = constant$13(value);
...
var formatLocale$4 = function(locale) {
...
function color$1() {
  ...
  legend.labelFormat = function (_) {
    ...
    labelFormat = typeof _ === 'string' ? format$1(_) : _;
    return legend;
  };
...
const BAR_FILL_COLOR = "#2b313c";
// const FILL_COLOR = '#5c6066';

function createScaleOrdinal(list) {
  return ordinal().domain(list).range([SENDING_COLOR, RECEIVING_COLOR]);
}

function createOrdinalGenerator(ordinal$$1) {
  return color$1().shape("path", symbol().type(symbolCircle).size(150)()).shapePadding(10)
...
function drawColorLegend(svg) {
  ...
  const ordinal$$1 = createScaleOrdinal(["OUTGOING", "INCOMING"]);
  const legendOrdinal = createOrdinalGenerator(ordinal$$1);

  return colorLineLegend.attr("transform", "translate(20,20)").call(legendOrdinal);
}

const createLineSize = maxDomain => linear$2().domain([0, maxDomain]).range([1, 6]);
...
function drawLinkGuideLegend({ svg, maxDomain }) {
  ...
  // Create New Legend
  svg.append("g").attr("class", "legendSizeLine").attr("transform", "translate(0, 500)");
  // .attr("opacity", 0)
  // .transition(t)
  // .delay(1000)
  // .attr("opacity", 1);

  var legendSizeLine = size().scale(lineSize).shape("line").orient("horizontal")
  ...
  .labelWrap(30).shapeWidth(40).labelAlign("start").shapePadding(10);

  svg.select(".legendSizeLine").call(legendSizeLine);
}

function createSvg$1(selector, settings = {}) {
  const {
    WIDTH: WIDTH$$1, HEIGHT: HEIGHT$$1, MARGIN: MARGIN$$1
  } = Object.assign({}, dimensions, settings);

  return select(selector).append("svg").attr('width', WIDTH$$1 + MARGIN$$1.left + MARGIN$$1.right).attr('height', HEIGHT$$1 + MARGIN$$1.top + MARGIN$$1.bottom).attr('class', settings.className || undefined).call(responsivefy).append('g').attr('transform', `translate(${MARGIN$$1.left}, ${MARGIN$$1.top})`);
}
...
function createSvgGroup(svg, className, id) {
  return svg.append('g').attr('class', className).attr('id', id);
}
...
var transform$2 = function(transform) {
  if (transform == null) return identity$12;
  var x0,
      y0,
...
function createStates(svg) {
  ...
  json("data/us.json", function (error, us) {
    ...
    states.selectAll("path").data(feature(us, us.objects.states).features).enter().append("path").attr("d", path).attr("class", "subunit-boundary").attr("fill", MAP_COLOR).attr("stroke", STATE_BORDER_COLOR);
  });
}

const createRadiusRange = maxDomain => linear$2().domain([0, maxDomain]).range([1, 6]);

function drawCircles({ data, svg, maxDomain }) {
	if (!svg) {
		svg = createSvg(".chart");
	}
	const radiusRange = createRadiusRange(maxDomain);

	const locationCircleGroup = svg.select(".locations-circles").size() > 0 ? svg.select(".locations-circles") : createSvgGroup(svg, "locations-circles", "locations-circles");
	var t = transition().duration(1000);

	var updatedLocations = data.map(l => Object.assign({}, l, {
		radiusSize: radiusRange(l.weight)
	}));

	var update = locationCircleGroup.selectAll("circle").data(updatedLocations, d => d.name);

	// Remove old circles
	update.exit().transition(t).attr("r", 0).remove();

	// Update any remaining circles to their new radius
	update.transition(t).delay(1000).attr("r", d => d.radiusSize);

	// Add new circles in
	update.enter().append("circle").attr("class", "location").attr("r", 0).attr("cx", d => d.x).attr("cy", d => d.y).style("fill", FILL_COLOR).style("stroke", "#252525").transition(t).delay(update.exit().size() ? 1000 : 0).attr("r", d => d.radiusSize);
}
...
const createFontRange = maxDomain => linear$2().domain([0, maxDomain]).range([5, 10]);

function drawLabels({ data, svg, store, maxDomain }) {
	if (!svg) {
		svg = createSvg$1(".chart");
	}
	...
	const locationLabelGroup = svg.select(".locations-label").size() > 0 ? svg.select(".locations-label") : createSvgGroup(svg, "locations-label", "locations-label");
	var t = transition().duration(1000);
	const chosenLocation = store.get("chosenLocation");

	var voronoiInstance = voronoi().extent([[-1, -1], [WIDTH + 1, HEIGHT + 1]]);
	...
	var updatedCells = cells.map(d => Object.assign({}, d, {
		orient: function () {
			...
			return angle === 0 ? "right" : angle === -1 ? "top" : angle === 1 ? "bottom" : "left";
		}()
	}));
	...
	// Remove old circles
	update.exit().transition(t).attr("font-size", "0.1em").remove();

	update.transition(t).delay(1000).attr("font-size", d => d.name === chosenLocation ? "1em" : fontRange(d.weight) >= 10 ? "1em" : `${fontRange(d.weight) / 10}em`);

	update.enter().append("text").attr("class", d => `location-text location-label--${d.orient}`).attr("x", d => d.x).attr("y", d => d.y).attr("dx", d => {
		if (d.orient === "left") {
			return "-0.5em";
	...
	}).attr("font-size", "0.1em").transition(t).delay(update.exit().size() ? 1000 : 0).attr("font-size", d => d.name === chosenLocation ? "1em" : fontRange(d.weight) >= 10 ? "1em" : `${fontRange(d.weight) / 10}em`).text(d => {
...
const createLineWidthRange = maxDomain => linear$2().domain([0, maxDomain]).range([1, 6]);
...
function drawLinks({ data, svg, maxDomain }) {
	if (!svg) {
		svg = createSvg$1(".chart");
	}
	...
	const lineGraph = svg.select(".lines").size() > 0 ? svg.select(".lines") : createSvgGroup(svg, "lines", "lines");
	const t = transition().duration(1000);

	var nextLineData = data.map(l => Object.assign({}, l, {
	...
	update.exit().transition(t).attr("opacity", 0).remove();

	update.transition(t).delay(1000).attr("stroke-width", d => d.strokeWidth).attr("stroke", d => d.target.color);

	update.enter().append("path").attr("d", d => lineFunction(d.lineData)).attr("opacity", 0).attr("fill", "none").attr("stroke", d => d.target.color).attr("stroke-width", d => d.strokeWidth).transition(t).delay(update.exit().size() ? 1000 : 0).attr("opacity", 1);
}
...
function drawData(store) {
	...
	const maxDomainWithoutLimit = maxDomainTarget >= maxDomainSource ? maxDomainTarget : maxDomainSource;
...
function getDataSource(store) {
  return new Promise((resolve, reject) => {
    ...
    if (store.get("locations")) {
      ...
      queue().defer(csv$1, "data/labels.csv", typeLocation)
      // .defer(csv, "data/trash.csv")
      .defer(json, "data/waste-cleaned.json").defer(json, "data/received-waste.json").await(saveData);
    }
  });
}
...
function initializeChart(selector) {
  var svg = select(`${selector}-chart`);
  var margin = { top: 30, right: 100, bottom: 30, left: 100 };
  var width = 400 - margin.left - margin.right;
  var height = 300 - margin.top - margin.bottom;

  if (svg.size() === 0) {
    svg = createSvg$1(selector, {
    ...
    select(`${selector}-title`).style('visibility', 'inherit');
  }
...
function drawBarChart({ data, selector }) {
  const maxColumns = 10;
  const sortedData = data.sort((a, b) => b.value - a.value).slice(0, maxColumns);

  const { svg, margin, width, height } = initializeChart(selector);

  var t = transition().duration(1000);
  var yScale = band().domain(sortedData.map(d => d.name)).range([0, height]).padding(0.2);

  var yAxis = svg.select(".bar-chart-y-axis").size() > 0 ? svg.select(".bar-chart-y-axis") : svg.append("g").attr("class", "bar-chart-y-axis").attr("transform", `translate(${0}, 0)`).call(axisLeft(yScale));

  var xScale = linear$2().domain([0, 100]).range([0, width]);

  var xAxis = svg.select(".bar-chart-x-axis").size() > 0 ? svg.select(".bar-chart-x-axis") : svg.append("g").attr("class", "bar-chart-x-axis").attr("transform", `translate(${0}, ${height})`).call(axisBottom(xScale));

  var column = svg.select(".bar-chart-columns").size() > 0 ? svg.select(".bar-chart-columns") : svg.append("g").attr("class", "bar-chart-columns");

  var update = column.selectAll("rect").data(sortedData.filter(d => d.value), d => d.name);

  update.exit().transition(t).attr("x", 0).attr("width", 0).remove();

  xScale.domain([0, max(sortedData, d => d.value)]);
  xAxis.transition(t).delay(1000).call(axisBottom(xScale));

  yAxis.transition(t).delay(1000).call(axisLeft(yScale));

  update.transition(t).delay(1000)
  // .attr("x", 0)
  .attr("y", d => yScale(d.name)).attr("width", d => xScale(d.value)).attr("height", d => yScale.bandwidth());
  // .attr("width", d => width - xScale(d.value));

  update.enter().append("rect").attr("x", 0).attr("width", 0).attr("y", d => yScale(d.name)).attr("height", d => yScale.bandwidth()).attr("fill", BAR_FILL_COLOR).transition(t).delay(update.exit().size() ? 2000 : 0).attr("width", d => xScale(d.value));
}

const svg = createSvg$1(".chart");
