...
var radius = sqrt$1().domain([0, 1e6]).range([0, 10]);
...
var ordinal = ordinal$1().domain(["SENT", "RECEIVED"]).range([SENDING_COLOR, RECEIVING_COLOR]);

svg.append("g").attr("class", "legendOrdinal").attr("transform", "translate(20,20)");

var legendOrdinal = color$1()
//d3 symbol creates a path-string, for example
//"M0,-8.059274488676564L9.306048591020996,
//8.059274488676564 -9.306048591020996,8.059274488676564Z"
.shape("path", symbol().type(triangle).size(150)()).shapePadding(10)
...
var markerArrow = svg.append('svg:defs').append('svg:marker').attr('id', 'marker_arrow').attr('markerHeight', 5).attr('markerWidth', 5).attr('markerUnits', 'strokeWidth').attr('orient', 'auto').attr('refX', 0).attr('refY', 0).attr('viewBox', '-5 -5 10 10').append('svg:path').attr('d', 'M 0,0 m -5,-5 L 5,0 L -5,5 Z').attr('fill', FILL_COLOR);
...
function createStates() {
  ...
  json("data/us.json", function (error, us) {
    ...
    states.selectAll("path").data(feature(us, us.objects.states).features).enter().append("path").attr("d", path).attr("fill", MAP_COLOR).attr('stroke', STATE_BORDER_COLOR);
...
function update(chosenLocation) {
  ...
  function drawData(locations, links) {
    var line = line().curve(bundle).x(d => d.x).y(d => d.y);

    var locationCircles = locationGroup.selectAll("circle").data(locations, d => d);

    locationCircles.enter().append("circle").attr('class', 'location').attr("r", 5).attr("cx", d => d.x).attr("cy", d => d.y).style("fill", FILL_COLOR)
    // .text(d => d.name)
    // .style("opacity", d => d.name === chosenLocation ? '1' : '0')
    .style("stroke", "#252525").merge(locationCircles);

    var locationText = locationGroup.selectAll('text').data(locations, d => d);

    locationText.enter().append("text").attr("class", "location-text").attr("x", d => d.x).attr("y", d => d.y).attr("dx", ".5em").attr("dy", "1em").text(d => {
    ...
    var maxDomain = max(links, l => +l.target.weight);
    ...
    var lines = lineGraph.selectAll("path").data(lineData, d => d);
