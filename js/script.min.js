...
function createOrdinalGenerator(ordinal) {
  return color$1().shape("path", symbol().type(circle$2).size(150)()).shapePadding(10)
...
function createColorLegend$$1(svg) {
...
function createSvg(selector) {
  return select(selector).append("svg").attr('width', WIDTH + MARGIN.left + MARGIN.right).attr('height', HEIGHT + MARGIN.top + MARGIN.bottom).call(responsivefy).append('g').attr('transform', `translate(${MARGIN.left}, ${MARGIN.top})`);
}
...
function drawLocationCircles(locations, settings) {
	const locationCircleGroup = svg$1.select(".locations-circles").size() > 0 ? svg$1.select(".locations-circles") : createSvgGroup(svg$1, "locations-circles", "locations-circles");
	var t = transition().duration(1000);

	var updatedLocations = locations.map(l => Object.assign({}, l, {
...
function drawLocationLabels(locations, settings) {
	const locationLabelGroup = svg$1.select(".locations-label").size() > 0 ? svg$1.select(".locations-label") : createSvgGroup(svg$1, "locations-label", "locations-label");
	var t = transition().duration(1000);
	const chosenLocation = _store$1.get("chosenLocation");

	var update = locationLabelGroup.selectAll("text").data(locations, d => d.name);

	// Remove old circles
	update.exit().transition(t).attr("font-size", "0.1em").remove();

	update.transition(t).delay(1000).attr("font-size", d => d.name === chosenLocation ? "1em" : settings.fontRange(d.weight) >= 10 ? "1em" : `${settings.fontRange(d.weight) / 10}em`);

	update.enter().append("text").attr("class", "location-text").attr("x", d => d.x).attr("y", d => d.y).attr("dx", ".5em").attr("dy", "1em").attr("font-size", "0.1em").transition(t).delay(update.exit().size() ? 1000 : 0).attr("font-size", d => d.name === chosenLocation ? "1em" : settings.fontRange(d.weight) >= 10 ? "1em" : `${settings.fontRange(d.weight) / 10}em`).text(d => {
...
function drawBetweenLines(lineData, settings) {
	const lineGraph = svg$1.select(".lines").size() > 0 ? svg$1.select(".lines") : createSvgGroup(svg$1, "lines", "lines");
	const t = transition().duration(1000);

	var nextLineData = lineData.map(l => Object.assign({}, l, {
	...
	update.transition(t).delay(1000).attr("stroke-width", d => d.strokeWidth);

	update.enter().append("path").attr("d", d => settings.lineFunction(d.lineData)).attr("opacity", 0).attr("fill", "none").attr("stroke", d => d.target.color).attr("stroke-width", d => d.strokeWidth).transition(t).delay(update.exit().size() ? 1000 : 0).attr("opacity", 1);
}

function drawLegend(settings) {
	var t = transition().duration(1000);

	var update = svg$1.select("g.legendSizeLine");
	...
	// Create New Legend
	svg$1.append("g").attr("class", "legendSizeLine").attr("transform", "translate(0, 500)");
	// .attr("opacity", 0)
	// .transition(t)
	// .delay(1000)
	// .attr("opacity", 1);

	var legendSizeLine = size().scale(settings.lineSize).shape("line").orient("horizontal")
	...
	svg$1.select(".legendSizeLine").call(legendSizeLine);
}

function drawData(locations, links, store) {
	_store$1 = store;
	svg$1 = _store$1.get('svg');
	var maxDomainTarget = max(links, l => +l.target.weight);
	var maxDomainSource = max(links, l => +l.source.weight);
	...
	var radiusRange = linear$2().domain([0, maxDomain]).range([3, 6]);

	var lineWidthRange = linear$2().domain([0, maxDomain]).range([1, 6]);

	var fontRange = linear$2().domain([0, maxDomain]).range([5, 10]);

	var curvedLine = line().curve(bundle).x(d => d.x).y(d => d.y);
	...
	var lineSize = linear$2().domain([0, maxDomain]).range([1, 6]);
...
function drawLocation(chosenLocation, store) {
  ...
  if (store.get("locations")) {
    filterData(null, store.get("locations"), store.get("trashData"), store.get("receivedTrashData"));
  } else {
    queue().defer(csv$1, "data/labels.csv", typeLocation)
    // .defer(d3.csv, "data/trash.csv")
    .defer(json, "data/waste-cleaned.json").defer(json, "data/received-waste.json").await(filterData);
  }
}
