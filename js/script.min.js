...
const SELECTED_CITY_COLOR = '#654321';
...
function createSvg() {
  return select("body").append("svg").attr("width", WIDTH).attr("height", HEIGHT);
}
...
function update(chosenLocation) {
  ...
  function drawData(locations, links) {
    ...
    var locationCircles = locationGroup.selectAll("circle").remove().data(locations, d => d);

    locationCircles.enter().append("circle").attr("class", "location").attr("r", d => radiusRange(d.weight))
    // .attr("r", d => 3)
    .attr("cx", d => d.x).attr("cy", d => d.y).style("fill", d => d.name === chosenLocation ? SELECTED_CITY_COLOR : FILL_COLOR)
    // .text(d => d.name)
    // .style("opacity", d => d.name === chosenLocation ? '1' : '0')
    .style("stroke", "#252525").merge(locationCircles);

    var locationText = locationGroup.selectAll("text").remove().data(locations, d => d);

    locationText.enter().append("text").attr("class", "location-text").attr("x", d => d.x).attr("y", d => d.y).attr("dx", ".5em").attr("dy", "1em").attr("font-size", d => d.name === chosenLocation ? "1em" : fontRange(d.weight) >= 10 ? "1em" : `${fontRange(d.weight) / 10}em`).attr("fill", d => d.name === chosenLocation ? SELECTED_CITY_COLOR : '#000').text(d => {
    ...
    var lines = lineGraph.selectAll("path").remove().data(lineData, d => d);

    lines.enter().append("path").attr("d", d => lineFunction(d.lineData)).attr("stroke", d => d.target.color).attr("stroke-width", d => lineWidthRange(+d.target.weight) + "px").attr("fill", "none").merge(lines);
